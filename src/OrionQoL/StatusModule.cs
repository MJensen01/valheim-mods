...
{
    /// <summary>
    /// The `orion.status` console command. Client-side only: Terminal's command table lives on
    ...
    internal sealed class StatusModule : FeatureModule
    {
        ...
        private static void RegisterCommand()
        {
            ...
            try
            {
                new Terminal.ConsoleCommand("orion.status",
                    "OrionQoL: world tier, catch-up settings and module state",
                    new Terminal.ConsoleEvent(Run));
                Log.LogInfo("[Status] console command 'orion.status' registered");
            }
            catch (Exception e)
            {
                _registered = false;
                Log.LogError("[Status] could not register orion.status: " + e);
            }
        }

        private static void Run(Terminal.ConsoleEventArgs args)
        {
            foreach (var line in OrionQoLPlugin.StatusLines())
            {
