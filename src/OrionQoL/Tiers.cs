...
namespace OrionQoL
{
    ...
    internal static class Tiers
    {
        ...
        {
            MaterialTiers = OrionQoLPlugin.BindSynced(Section, "MaterialTiers", DefaultMap,
            ...
            OrionQoLPlugin.ConfigChanged += OnConfigChanged;
            Parse();
        }
        ...
        private static void OnConfigChanged(ConfigEntryBase entry)
        {
            ...
            _validated = false;
            ValidateOnce();
            OrionQoLPlugin.Log.LogInfo("[Tiers] material map reloaded: " + _map.Count + " entries");
        }
        ...
        private static void Parse()
        {
            ...
            foreach (var chunk in raw.Split(new[] { ',', ';', '\n' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if (colon <= 0)
                {
                    OrionQoLPlugin.Log.LogWarning("[Tiers] ignoring malformed entry '" + piece + "' (want Name:tier)");
                    continue;
                }
                ...
                {
                    OrionQoLPlugin.Log.LogWarning("[Tiers] ignoring '" + piece + "': tier must be 0.." + Frontier.MaxTier);
                    continue;
                }
        ...
        public static void ValidateOnce()
        {
            ...
            if (_unknown.Count > 0)
                OrionQoLPlugin.Log.LogWarning("[Tiers] " + _unknown.Count + " configured material(s) not in ObjectDB: " +
                                              string.Join(", ", _unknown.ToArray()));
            else
                OrionQoLPlugin.Log.LogInfo("[Tiers] " + _map.Count + " materials, all present in ObjectDB");
        }
