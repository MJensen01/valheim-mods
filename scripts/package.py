...
`thunderstore/<Mod>/` and only fills in the DLL, so editing those files is the way to change a
mod's Thunderstore listing content — this script does not hardcode any of it.

Usage (after `dotnet build -c Release` for both projects):
    python scripts/package.py [--config Release]

Output: dist/<Name>-<version>.zip, one per mod under thunderstore/.
"""
import argparse
import json
import os
...
def build_one(mod, configuration):
    ...
    if not os.path.exists(dll_path):
        raise SystemExit(
            f"error: {dll_path} not found — run `dotnet build -c {configuration} src/{name}` first"
        )
...
def main():
    ...
    parser.add_argument("--config", default="Release", help="Build configuration the DLLs were built with (informational only; DLLs are always read from bin/)")
    args = parser.parse_args()

    os.makedirs(DIST, exist_ok=True)
    outputs = [build_one(mod, args.config) for mod in MODS]
    print(f"\nBuilt {len(outputs)} package(s) into {DIST}")
